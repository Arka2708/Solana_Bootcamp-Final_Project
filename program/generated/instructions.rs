...
pub enum MyDigitalArtNftInstruction {
/// Accounts:
...
impl MyDigitalArtNftInstruction {
    pub fn unpack(input: &[u8]) -> Result<Self, ProgramError> {
        let (&variant, rest) = input.split_first().ok_or(MyDigitalArtNftError::InvalidInstruction)?;

        Ok(match variant {
			0 => Self::Mint(MintArgs::try_from_slice(rest).unwrap()),
			1 => Self::Transfer,
			2 => Self::Burn,
			_ => return Err(MyDigitalArtNftError::InvalidInstruction.into())
        })
    }
}