...
use crate::generated::errors::MyDigitalArtNftError;
use crate::generated::instructions::MyDigitalArtNftInstruction;
...
impl Processor {
    pub fn process(
        ...
        let instruction = MyDigitalArtNftInstruction::unpack(data)?;

        match instruction {
			MyDigitalArtNftInstruction::Mint(args) => {
			...
			MyDigitalArtNftInstruction::Transfer => {
				msg!("Instruction: Transfer");
				Self::process_transfer(program_id, accounts)
			}
			MyDigitalArtNftInstruction::Burn => {
...
/// - image_url: [String] 
	pub fn process_mint(
		...
		let digital_art_info = next_account_info(account_info_iter)?;
		...
		// Derive PDAs
		let (digital_art_pubkey, digital_art_bump) = Pubkey::find_program_address(
			&[b"digital_art", mint_info.key.as_ref()],
			program_id,
		);

		// Security Checks
		if fee_payer_info.is_signer != true {
			return Err(MyDigitalArtNftError::InvalidSignerPermission.into());
		}
		...
		if *digital_art_info.key != digital_art_pubkey {
			return Err(MyDigitalArtNftError::NotExpectedAddress.into());
		}
		...
		let space = DigitalArtMetadata::LEN;
		let rent = Rent::get()?;
		let rent_minimum_balance = rent.minimum_balance(space);
		...
		invoke_signed(
			&create_account(
				&fee_payer_info.key,
				&digital_art_info.key,
				rent_minimum_balance,
				space as u64,
				program_id,
			),
			&[fee_payer_info.clone(), digital_art_info.clone()],
			&[&[b"digital_art", mint_info.key.as_ref(), &[digital_art_bump]]],
		)?;


		// Security Checks
		if *fee_payer_info.owner != Pubkey::from_str("11111111111111111111111111111111").unwrap() {
			return Err(MyDigitalArtNftError::WrongAccountOwner.into());
		}
		...
		if mint_info.data_len() != spl_token::state::Mint::LEN {
			return Err(MyDigitalArtNftError::InvalidAccountLen.into());
		}

		if digital_art_info.data_len() != DigitalArtMetadata::LEN {
			return Err(MyDigitalArtNftError::InvalidAccountLen.into());
		}
		...
		let digital_art = &mut AccountPDA::new(
			&digital_art_info,
			try_from_slice_unchecked::<DigitalArtMetadata>(&digital_art_info.data.borrow()).unwrap(),
			digital_art_bump,
		);
		...
		digital_art.data.serialize(&mut &mut digital_art_info.data.borrow_mut()[..])?;
