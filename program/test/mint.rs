...
mod mint {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("my_digital_art_nft", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let title: String = Default::default();
		let artist: String = Default::default();
		...
		let (digital_art_pubkey, _) = Pubkey::find_program_address(
			&[b"digital_art", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(digital_art_pubkey, false),
		...
		let data = MyDigitalArtNftInstruction::Mint(crate::generated::instructions::MintArgs{
			title,
			artist,
			description,
			image_url,
